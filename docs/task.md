...
## タスク6: UI改善3点（画像削除・表示名の言語切り替え・試薬リクエスト未翻訳箇所）
依存: なし / 難易度: 低〜中
**実装タイミング注意: ラボのアクセスが活発なため、利用が落ち着いたタイミングで実装すること（ユーザー指示）。**

### 6-1. アイテム画像の削除
- [ ] `ItemDetail.tsx` の画像プレビュー横に「画像を削除」ボタンを追加する。
- [ ] クリックで `formData.imageUrl` を空にし、フォーム送信（保存）時に `PATCH /api/items/:id` へ反映する。
- [ ] バックエンドは変更不要（既に `imageUrl` の空文字/null更新を受け付ける）。

### 6-2. Englishモード時、表示名を英語名にする
- [ ] `frontend/src/utils/` に `getDisplayName(name, englishName, lang)` ヘルパーを新設する。
  - 仕様: lang が `'en'` かつ `englishName` が存在する場合のみ英語名を主表示にする。`englishName` が空/未設定の場合は英語モードでも日本語名 (`name`) にフォールバックする（ユーザー確認済み）。
  - 主表示・補足表示の入れ替え: 現在「日本語名を主表示・英語名を補足」の構成を、Englishモードでは「英語名を主表示・日本語名を補足」に反転させる。
- [ ] 以下の主表示箇所を置き換える: `Home.tsx`（item / reagent 名表示）, `Manage.tsx`（テーブルのアイテム名列）, `Consume.tsx`（カード見出し）, `Restock.tsx`（カード見出し・試薬リクエストカード）, `ReagentRequest.tsx`（検索結果カード・選択済み表示）。

### 6-3. 試薬発注リクエスト画面の未翻訳箇所
- [ ] `locales/types.ts` の `reagentRequest` に以下を追加: `existingTabLabel`（既存から選ぶタブ）, `newTabLabel`（新規試薬を発注タブ）, `catalogNumberPlaceholder`（カタログ番号の入力例）。
- [ ] `ja.ts` / `en.ts` に値を追加する。
- [ ] `ReagentRequest.tsx` のハードコードされた日本語（`"🔍 既存から選ぶ"` `"＋ 新規試薬を発注"` `"例: 194-09995"`）を上記キーに置き換える。
...
## タスク7（案・未着手）: 在庫がある部屋のオフライン管理対応
背景: 在庫がある部屋がオフライン環境（サーバーと常時通信できない）。その部屋のデバイスは、ラボ内を移動するなどして**一時的に**サーバーと同じネットワークに入ることがある（ユーザー確認済み）。
**実装タイミング注意: 他タスク（タスク6）と同様、ラボの利用が落ち着いたタイミングまで着手しないこと（ユーザー指示）。**

- [ ] PWA化: Service Worker + manifest でアプリ自体をオフラインでも開けるようにする。
- [ ] オフライン中の操作（数量変更・ステータス変更など）をIndexedDBにキューとして保存する。
- [ ] オンライン復帰（`navigator.onLine` や SSE再接続など）を検知し、キューを順にサーバーへ再送する。
- [ ] 競合方針: 数量変更は既にサーバー側で差分（delta）加算のため、複数拠点のオフライン変更が自然にマージされる。名前・しきい値などの**絶対値編集**が競合した場合は「後勝ち（last-write-wins）」で割り切る（ユーザー確認済み・追加の競合検知UIは作らない）。
